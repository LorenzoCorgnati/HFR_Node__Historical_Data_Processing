...
try
    for network_idx=1:numNetworks
        ...
        try
            toBeConvertedTotals_selectquery = ['SELECT * FROM total_input_tb WHERE datetime>=' '''' startDate ''' AND datetime<' '''' endDate ''' AND network_id = ' '''' network_data{network_idx,network_idIndex} ''' AND NRT_processed_flag = 0'];
