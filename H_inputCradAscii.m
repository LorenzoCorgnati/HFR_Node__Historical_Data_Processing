%% H_inputCradAscii.m
% This application lists the input crad_ascii (WERA radials) files pushed 
% by the HFR data providers and inserts into the a proper structure the 
% information needed for the combination of radial files into totals and 
% for the generation of the radial and total data files into the European 
% standard data model.
...
disp(['[' datestr(now) '] - - ' 'H_inputCradAscii started.']);
    ...
    for network_idx=1:numNetworks
        ...
        for station_idx=1:numStations
            if(~isempty(station_data{station_idx,inputPathIndex}))
                ...
                try
                    cradFiles = rdir([station_data{station_idx,inputPathIndex} filesep '**' filesep '*.crad_ascii']);
...
if(iCradDB_err==0)
    disp(['[' datestr(now) '] - - ' 'H_inputCradAscii successfully executed.']);
end